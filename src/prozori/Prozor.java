...
public class Prozor extends Frame {
		...
		b.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) throws IllegalArgumentException {
				...
				if(cmp.length()>0 && start.length()>0 && end.length()>0)
				{
					...
					Prozor.this.scena.setStock(new Stock(cmp, t1.getTime()/1000, t2.getTime()/1000));
					if(Prozor.this.scena.stock.brSveca() > 0)
						setup.dispose();
					else
	...
	private void populateWindow() {
		...
		prikazi.addActionListener(a->{
			Prozor.this.scena.prikaziSvece=true;
			Prozor.this.scena.repaint();
		});
		
		sakriji.addActionListener(a->{
			Prozor.this.scena.prikaziSvece=false;
			Prozor.this.scena.repaint();
		});
		...
		p1.addActionListener(ae->{
			Dialog d=new Dialog(Prozor.this,ModalityType.APPLICATION_MODAL);
			...
			bb.addActionListener(aee->{
				try {
					...
					d.dispose();
					scena.maNiz=new Ma().dohvati(Prozor.this.scena.stock, n);
					scena.ma=true;
					scena.repaint();
		...
		p2.addActionListener(ae->{
			...
			bb.addActionListener(aee->{
				try {
					int n=Integer.valueOf(t.getText());
					d.dispose();
					scena.emaNiz=new Ema().dohvati(Prozor.this.scena.stock, n);
					scena.ema=true;
					scena.repaint();
